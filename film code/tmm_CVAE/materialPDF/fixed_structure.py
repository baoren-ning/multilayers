...
from fix_structure_model import Generator, Generator1
...
Hfo21_path = filename + '/Hfo21.csv'
nk_Hfo21 = np.genfromtxt(Hfo21_path, delimiter=',')
refractive_index_Hfo21 = (nk_Hfo21[:, 1] + nk_Hfo21[:, 2] * 1j).reshape(1, -1)
nk_material_Hfo21 = torch.from_numpy(refractive_index_Hfo21)

vo2_path = filename + '/vo2.csv'
nk_vo2 = np.genfromtxt(vo2_path, delimiter=',')
refractive_index_vo2 = (nk_vo2[:, 1] + nk_vo2[:, 2] * 1j).reshape(1, -1)
nk_material_vo2 = torch.from_numpy(refractive_index_vo2)
    ...
    for i in range(epoch):
        ...
        for step, (x, y) in enumerate(train_loader):
            ...
            if continuous_nk:
                ...
                M_matrix = torch.cat([M_matrix, nk_material_vo2, nk_material_Hfo21, nk_material_ag], 0)
                T, M = M_T(out_t, M_matrix, nk_dim, continuous_nk)
                c_id = torch.where(material_id == 2)
                M_matrix[c_id] = nk_material
                M_matrix[-3,:] = nk_material
            ...
            O_l_A = 1 - O_l_R - O_l_T
            input_h = torch.cat([wl[:, 1101:].reshape(1, -1).to(device), O_A[0:1, 1101:]], dim=0).view(1, 1, 2, nk_dim_8_13)
            input_l = torch.cat([wl[:, 1101:].reshape(1, -1).to(device), O_l_A[0:1, 1101:]], dim=0).view(1, 1, 2, nk_dim_8_13)
            em_h = cal_emittance_rad(lamda, input_h, 373)
            em_l = cal_emittance_rad(lamda, input_l, 300)
            alpha = AM15(torch.tensor(wl_03_25), O_A[0:1, 0:1101])
            deta = em_h - em_l
            error =  (2.5)**(5*alpha)+torch.abs(1/deta)*3
            # error =  80*(alpha**2-0.2*alpha+0.01)+torch.abs(1/deta)
            # error = 1*alpha + 50*(1-deta)
...
if __name__ == '__main__':
    ...
    plt.subplot(221)
    plt.plot((wl[:,0:1101] * 10 ** 9).reshape(-1), O[0, 0:1101].detach().cpu().numpy(), color='red', label='H_vo2_03_25')
    plt.legend()
    plt.ylim((0, 1))
    alpha = AM15(torch.tensor(wl_03_25), O[0:1, 0:1101])
    print('高温太阳吸收比：', alpha)
    plt.subplot(223)
    plt.plot((wl[:,0:1101] * 10 ** 9).reshape(-1), O_l[0, 0:1101].detach().cpu().numpy(), color='blue', label='L_vo2_03_25')
    plt.legend()
    plt.ylim((0, 1))
    plt.subplot(222)
    plt.plot((wl[:, 1101:] * 10 ** 9).reshape(-1), O[0, 1101:].detach().cpu().numpy(), color='red', label='H_vo2_8_13')
    plt.legend()
    plt.ylim((0, 1))
    plt.subplot(224)
    plt.plot((wl[:, 1101:] * 10 ** 9).reshape(-1), O_l[0, 1101:].detach().cpu().numpy(), color='blue', label='L_vo2_8_13')
    plt.ylim((0, 1))
    plt.legend()
    plt.show()
    input_h = torch.cat([wl[:, 1101:].reshape(1, -1).to(device), O[0:1, 1101:]], dim=0).view(1, 1, 2, nk_dim_8_13)
    input_l = torch.cat([wl[:, 1101:].reshape(1, -1).to(device), O_l[0:1, 1101:]], dim=0).view(1, 1, 2, nk_dim_8_13)
    em_h = cal_emittance_rad(lamda, input_h, 373)
    em_l = cal_emittance_rad(lamda, input_l, 300)
