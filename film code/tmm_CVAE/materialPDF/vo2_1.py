...
# wl = np.linspace(2600, 16000, nk_dim) * (10 ** (-9))  # 波长范围
...
def M_T1(out_t, out_nk, nk_dim, continuous_nk):
    ''''
    out_t：神经网络输出的膜层厚度，需要调整为两边是无限厚的空气层
    out_nk：n+jk形式，需要调整为两边为空气时的折射[1,1+1j,2+0.4j........,1]
    '''
...
def model_train(iteration, continuous_nk):
    ...
    for i in range(iteration):
        ...
        if continuous_nk:
            T, M, material_id, p_value, mse = train(model, 400, continuous_nk, train_loader)
            T_film.append(T)
            MSE.append(mse)
            M_matrix.append(M)
...
if __name__ == '__main__':
    ...
    M_ag = M_matrix_l.reshape(1,-1,nk_dim)
    M_ag = M_ag[:,1:-1,:]
    M_ag = torch.cat([M_ag,nk_material_ag.reshape(1,1,-1)], 1)

    T_ag = T_film[id]
    T_ag = T_ag[:,1:-1]
    t_ag = torch.tensor(5* 10 ** -9).view(1,-1).to(device)
    T_ag = torch.cat([T_ag,t_ag],1)

    T_ag_l, M_ag_l = M_T1(T_ag, M_ag, nk_dim, continuous_nk)
    O_R_l_ag = tmm('s', M_ag_l.reshape(1, -1, nk_dim), T_ag_l, theta, wl, device='cuda')[mode]
    O_T_l_ag = tmm('s', M_ag_l.reshape(1, -1, nk_dim), T_ag_l, theta, wl, device='cuda')['T']
    O_l_ag = 1 - O_R_l_ag - O_T_l_ag
    ...
    plt.plot(wl * 10 ** 9, O_l_ag[0, :].detach().cpu().numpy(), color='yellow', label='L_vo2_ag')
    plt.ylim((0, 1))
    plt.legend()
    plt.show()
